    ...
    protected function getTableColumns(): array
    {
        return [
            TextColumn::make('days_overdue')
                ->label('Status')
                ->badge()
                ->getStateUsing(fn ($record) => Carbon::parse($record->due_date)->diffInDays(now()).' days'
                )
                ->color(fn ($record) => Carbon::parse($record->due_date)->diffInDays(now()) > 60 ? 'danger' :
                    (Carbon::parse($record->due_date)->diffInDays(now()) > 30 ? 'warning' : 'gray')
                )
                ->icon(fn ($record) => Carbon::parse($record->due_date)->diffInDays(now()) > 60
                        ? 'heroicon-o-exclamation-triangle'
                        : 'heroicon-o-clock'
                )
                ->extraAttributes(fn ($record) => Carbon::parse($record->due_date)->diffInDays(now()) > 90
                        ? ['class' => 'animate-pulse']
                        : []
                ),
